...
export default function ItemDetailPage() {
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try {
        const query = poeApi.buildItemQuery(qName, { online: true })
        const search = await poeApi.searchItems(league, query)
        if (!search?.id) { if(!cancelled) setListings([]); return }
        const details = await poeApi.getItemDetails(search.id, search.result)
        if (cancelled) return
        const lite: ListingLite[] = details.map(d=>({
          id: d.id,
          price: d.listing.price ? { amount: d.listing.price.amount, currency: d.listing.price.currency } : undefined,
          seller: d.listing.account?.name,
          indexed: d.listing.indexed,
          whisper: d.listing.whisper,
          rarity: d.item.rarity,
          icon: d.item.icon,
          name: d.item.name,
          typeLine: d.item.typeLine,
          baseType: d.item.baseType,
          properties: d.item.properties,
          implicitMods: d.item.implicitMods,
          explicitMods: d.item.explicitMods,
        })).slice(0,8)
        setListings(lite)
  ...
  const rarityColor = (rarity?: string) => {
    if (!rarity) return '#c8c8c8'
    const r = rarity.toLowerCase()
    if (r==='unique') return '#af6025'
    if (r==='rare') return '#ffff77'
    if (r==='magic') return '#8888ff'
    return '#c8c8c8'
  }
  ...
  const listingStats = useMemo(()=>{
    ...
    const valuesChaos = priced.map(l=>l.price!.amount) // assume same currency chaos or divine mostly; advanced conversion omitted
    const sorted = [...valuesChaos].sort((a,b)=>a-b)
    const mid = sorted[Math.floor(sorted.length/2)]
    const avg = valuesChaos.reduce((a,b)=>a+b,0)/valuesChaos.length
    const low = sorted[0]
    const high = sorted[sorted.length-1]
    return { median: mid, average: avg, low, high, currency: priced[0].price!.currency }
  }, [listings])
  ...
  return (
    <div style={{padding:24, display:'flex', flexDirection:'column', gap:24}}>
      ...
      <div style={{display:'flex',gap:28,alignItems:'flex-start'}}>
        {/* LEFT: Price Action & Metrics */}
        <div style={{flex: '1 1 820px', minWidth:700}}>
          <div style={{background:'#141414',padding:'20px 22px 18px',border:'1px solid #272727',borderRadius:8,boxShadow:'0 2px 4px rgba(0,0,0,.4)'}}>
            ...
            <div style={{display:'flex',flexWrap:'wrap',gap:28,marginTop:24,fontSize:13}}>
              ...
              {listingStats && (
                <div style={{display:'flex',flexDirection:'column',gap:6}}>
                  <div style={{opacity:.55}}>Sample Prices</div>
                  <div style={{display:'flex',gap:14,fontSize:12}}>
                    <span>Low <strong>{listingStats.low.toFixed(1)} {listingStats.currency}</strong></span>
                    <span>Med <strong>{listingStats.median.toFixed(1)}</strong></span>
                    <span>Avg <strong>{listingStats.average.toFixed(1)}</strong></span>
                    <span>High <strong>{listingStats.high.toFixed(1)}</strong></span>
                  </div>
                </div>
              )}
        ...
        <div style={{width:420,display:'flex',flexDirection:'column',gap:20}}>
          <div style={{background:'#161616',padding:'18px 20px 14px',border:'1px solid #2a2a2a',borderRadius:8}}>
            ...
            <div style={{display:'flex',gap:14,alignItems:'center',marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,opacity:.6}}>{item.baseType || item.name}</div>
              </div>
            </div>
            {item.implicitModifiers?.length>0 && (
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,opacity:.55,marginBottom:4}}>Implicit Summary</div>
                {item.implicitModifiers.map((m:string,i:number)=>(<div key={i} style={{fontSize:11,color:'#8888ff'}}>{m}</div>))}
              </div>
            )}
            {item.explicitModifiers?.length>0 && (
              <div style={{marginBottom:12}}>
                <div style={{fontSize:11,opacity:.55,marginBottom:4}}>Explicit Summary</div>
                {item.explicitModifiers.map((m:string,i:number)=>(<div key={i} style={{fontSize:11,color:'#ffff77'}}>{m}</div>))}
              </div>
            )}
            <div style={{fontSize:10,opacity:.4}}>Data source: poe.ninja overview • real listings sampled below</div>
          </div>
          ...
          <div style={{background:'#161616',padding:'18px 20px 16px',border:'1px solid #2a2a2a',borderRadius:8,display:'flex',flexDirection:'column',gap:10}}>
            ...
            {listings && listings.length>0 && (
              <div style={{display:'flex',flexDirection:'column',gap:6,maxHeight:320,overflow:'auto',paddingRight:4}}>
                {listings.map(l=>{
                  ...
                  return (
                    <div key={l.id} style={{display:'flex',flexDirection:'column',gap:4,padding:'8px 10px',background:'#1d1d1d',border:'1px solid #272727',borderRadius:6}}>
                      <div style={{display:'flex',gap:8,alignItems:'center'}}>
                        {l.icon && <img src={l.icon} alt="" style={{width:30,height:30,objectFit:'contain'}} />}
                        <div style={{flex:1}}>
                          <div style={{fontSize:12,color:rarityColor(l.rarity)}}>{l.name || l.typeLine || l.baseType}</div>
                          <div style={{fontSize:10,opacity:.55}}>{price} • {formatAge(l.indexed)} • {l.seller}</div>
                        </div>
                        <button onClick={()=>navigator.clipboard.writeText(l.whisper)} style={{background:'none',border:'1px solid #333',color:'#bbb',fontSize:10,padding:'4px 6px',borderRadius:4,cursor:'pointer'}}>Copy</button>
                      </div>
                      {(l.implicitMods || l.explicitMods) && (
                        <div style={{display:'flex',flexDirection:'column',gap:2}}>
                          {l.implicitMods && l.implicitMods.slice(0,2).map((m,i)=>(<div key={'im'+i} style={{fontSize:10,color:'#8888ff'}}>{m}</div>))}
                          {l.explicitMods && l.explicitMods.slice(0,3).map((m,i)=>(<div key={'em'+i} style={{fontSize:10,color:'#ffff77'}}>{m}</div>))}
                        </div>
                      )}
                    </div>
                  )
            ...
            <div style={{fontSize:10,opacity:.4}}>Prices are snapshot; refine via full trade search.</div>
          </div>
        </div>
      </div>
    </div>
